...
for gender, styles in STYLES.items():
    for style in styles:
        ...
        if count < 2:
            raise SystemExit(f"No foreground component: {style}")
        ...
        if not (.08 < coverage < .58):
            raise SystemExit(f"Unsafe coverage {coverage:.3f}: {style}")
        if corner > 2:
            raise SystemExit(f"Foreground reaches corners ({corner:.2f}): {style}")
        bbox = (int(xs.min()), int(ys.min()), int(xs.max() + 1), int(ys.max() + 1))
        mask.save(OUT / f"{style}.png")
        print(style, f"coverage={coverage:.3f}", f"bbox={bbox}", f"corner={corner:.2f}")
