...
def exact(path,mode='RGBA'):
 ...
 if im.size!=SIZE: raise SystemExit(f'BAD SIZE {path}: {im.size}')
 return im
...
def clean_subject(im):
 ...
 # Preserve detached limb pieces only inside the body's vertical span and close to its silhouette.
 for i in range(1,n):
  if i==biggest: continue
  x,y,w,h,area=map(int,stats[i]); cx=x+w/2; cy=y+h/2
  limb_zone=(my+mh*.16)<=cy<=(my+mh+12) and (mx-95)<=cx<=(mx+mw+95)
  if limb_zone and area>=120: keep|=labels==i
...
def build_person(gender,bg):
 ...
 scale=min(max_w/subject.width,max_h/subject.height); nw,nh=int(subject.width*scale),int(subject.height*scale); subject=subject.resize((nw,nh),Image.Resampling.LANCZOS)
 left=(SIZE[0]-nw)//2; top=SIZE[1]-nh-35
...
def body_box(person):
 a=np.asarray(person.getchannel('A')); ys,xs=np.where(a>24); return int(xs.min()),int(ys.min()),int(xs.max()+1),int(ys.max()+1)
...
def skin_layer(gender,base):
 # Do not reuse legacy masks after geometry normalization; transparent layers prevent clothes/background recoloring.
...
def register_hair(im,person):
 ...
 tx=int(fx+fw/2-nw/2); ty=int(fy-nh*.30); canvas=Image.new('RGBA',SIZE,(0,0,0,0)); canvas.alpha_composite(crop,(tx,ty)); return canvas
