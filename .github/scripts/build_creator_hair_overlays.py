...
def detect_face(img):
    ...
    try:
        ...
        if faces is not None and len(faces):
            # choose the face nearest the image centre
...
def fit_layer(rgba, person_mask, target_h=.86, x_center=.52):
    ...
    # transform the WHOLE source using the person's bbox as reference so hair aligns with the canonical body
...
for gender,ss in styles.items():
    for style in ss:
        for color in colors:
            ...
            # Keep hair above/around the face, never the torso/body. Long female styles keep side strands.
            ...
            # Punch the face/neck out of the overlay so a hairstyle can NEVER cover the face.
            ...
            # For men, discard anything below the ears. For women allow only side strands below the chin.
            if gender=='male':
                hair[int(fy+1.02*fh):,:]=0
            else:
                ycut=int(fy+1.0*fh)
                if ycut<h:
                    central=(np.indices((h-ycut,w))[1] > int(cx-.42*fw)) & (np.indices((h-ycut,w))[1] < int(cx+.42*fw))
